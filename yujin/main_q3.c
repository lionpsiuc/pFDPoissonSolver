...
int main(int argc, char** argv) {
  ...
  if (cart_rank == 0) {
    ...
    /* Also write each process's local solution for verification */
    ...
    for (int i = 0; i < nprocs; i++) {
      ...
      /* Create a temporary grid with just this process's part */
      double temp_grid[maxn][maxn];
      init_full_grids(temp_grid, b, f);

      /* Copy the relevant part from global solution */
      for (int col = ls; col <= le; col++) {
        for (int row = 0; row <= ny + 1; row++) {
          temp_grid[col][row] = global_a[col][row];
        }
      }

      /* Write this process's part to file */
      write_grid(local_filename, temp_grid, nx, ny);
    }
    ...
    printf("\nTo visualize the solution, run the separate Python script "
           "'plot_solution.py'\n");
  }
