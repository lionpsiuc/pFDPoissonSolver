...
void init_cart2d(Cart2D *cart2d, MPI_Comm comm_old, int nx, int ny) {
    ...
    if (cart2d->coords[0] == cart2d->dims[0] - 1) {
        ...
        if (cart2d->local_size[0] <= 0) cart2d->local_size[0] = 1; // Ensure at least one point
    }
    ...
    if (cart2d->coords[1] == cart2d->dims[1] - 1) {
        ...
        if (cart2d->local_size[1] <= 0) cart2d->local_size[1] = 1; // Ensure at least one point
    }
...
void sweep2d(double a[][maxn], double f[][maxn], double b[][maxn], int nx, int ny, Cart2D *cart2d) {
    ...
    for (i = 1; i <= cart2d->local_size[0]; i++) {
        for (j = 1; j <= cart2d->local_size[1]; j++) {
            b[i][j] = 0.25 * (
                a[i-1][j] + a[i+1][j] + a[i][j-1] + a[i][j+1] - h*h*f[i][j]
            );
        }
    }
}
